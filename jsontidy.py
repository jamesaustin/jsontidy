...
def dump(data, stream, indent):
    def tab(d):
        return ' ' * (indent * d)
    def config_list(o, d):
        if (LIST_OF_NUMBERS and all(isinstance(x, (int, float)) and not isinstance(x, bool) for x in o)) or \
            (LIST_OF_BOOLS and all(isinstance(x, bool) for x in o)) or \
            (LIST_OF_LISTS_OF and all(isinstance(x, list) and can_compact_list(x) for x in o)) or \
            (LIST_SINGLES and len(o) == 1) or \
            (EMPTY and len(o) == 0):
            return ('[', ', ', ']', False, d)
        else:
            return ('[\n', ',\n', '\n' + tab(d) + ']', True, d+1)
    def config_dict(o, d):
        if (DICT_MAX_ELEMENTS >= len(o)) and (\
            (DICT_OF_NUMBERS and all(isinstance(x, (int, float)) and not isinstance(x, bool) for x in o)) or \
            (DICT_OF_BOOLS and all(isinstance(x, bool) for x in o)) or \
            (EMPTY and len(o) == 0)):
            return ('{', '"{}": ', ', ', '}')
        else:
            return ('{\n', tab(d+1) + '"{}": ', ',\n', '\n' + tab(d) + '}')
